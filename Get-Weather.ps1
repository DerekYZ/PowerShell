...
[CmdletBinding()] #<<-- This turns a regular function into an advanced function
      param([string]$location = (Read-host "Please input location: 'City, ST' or 'City, State'"),
...
$data = @(#array name
  for ($i=0; $i -lt $days+1; $i++) {
      ...
      else {if ($days -eq 1) {#array [1] for one day forecast because only one element in condition.text
                  [pscustomobject]@{
                        Location=("$location");
